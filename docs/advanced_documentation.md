...
(Some of these mentioned items are similar to other items which are included by default, e.g. `issues_authored` is similar to `issues`. Both use the GitHub search API for public issue counts; with [private access](fork.md#private-contributions-support) private issues are included as well. The `issues_authored` item additionally supports the `repo` and `owner` filters.)
